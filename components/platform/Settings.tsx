...
const getIconBackground = (name: SettingsIconName) => {
  if (isAndroid) return undefined;

  const backgrounds: Partial<Record<SettingsIconName, string>> = {
...
const getIconProps = (name: SettingsIconName) => {
  return {
    name:
      name === 'security'
    ...
    backgroundColor: getIconBackground(name),
  } as PlatformListItemProps['leftIcon'];
};
...
export const SettingsListItem: React.FC<SettingsListItemProps> = props => {
  ...
  return (
    <PlatformListItem
      ...
      isFirst={position === 'first' || position === 'single'}
      isLast={isLast}
    />
  );
};
